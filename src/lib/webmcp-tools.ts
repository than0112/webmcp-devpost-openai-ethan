  ...
  return [
    {
      ...
      execute: (input: SearchLostItemsInput, context) => {
        ...
        const allRanked = rankItems(sourceItems, searchInput, { strictFilters: false });
        const relevanceFloor = allRanked[0] ? Math.max(0.4, allRanked[0].confidence * 0.55) : 0;
        const ranked = allRanked.filter((result) => result.confidence >= relevanceFloor);
    ...
    },
    {
      ...
      execute: (input: CompareItemsInput, context) => { context?.signal?.throwIfAborted(); const session = input.session_id ? requireSession(store, input.session_id) : null; if (input.session_id && !session) return staleSessionError(input.session_id); const clues = mergeSearchClues(session?.clues ?? [], cluesFromKnown(input.known_clues, input.negative_clues)); const candidates = items.filter((item) => input.item_ids.includes(item.id)); const [best, ...alternatives] = compareItemsWithClues(candidates, clues); if (!best) return { error: { code: "no_candidates", message: "No valid candidate items." } }; if (session) publishSession(store, callbacks, investigationReducer(session, { type: "best_match", sessionId: session.id, itemId: best.item.id })); callbacks.onHighlight(best.item.id); callbacks.onActivity({ tool: "compare_items", message: `${Math.round(best.score * 100)}% match · ${best.item.id}`, state: "done" }); return { best_match: matchShape(best), alternatives: alternatives.map(matchShape) }; },
    },
    {
    ...
    },
    {
      ...
      execute: ({ item_id }: { item_id: string }, context) => { context?.signal?.throwIfAborted(); const item = items.find((candidate) => candidate.id === item_id); if (!item) return { error: { code: "item_not_found", message: "Item not found." } }; const session = store.get(); if (session) publishSession(store, callbacks, investigationReducer(session, { type: "request_confirmation", sessionId: session.id, itemId: item_id })); callbacks.onClaim(item); callbacks.onActivity({ tool: "request_claim", message: "Waiting for human", state: "active" }); return { status: "confirmation_required", item_id, message: "Human confirmation is required to claim this item." }; },
    },
  ];
}
