  ...
  return (
    <aside className={collapsed ? "activity-panel collapsed" : "activity-panel"} aria-label="Agent activity">
      ...
      {!collapsed && <div className="activity-body">
        ...
        {entries.length === 0 ? <div className="activity-empty"><p>No agent calls yet.</p><button onClick={onDemo}>Run demo flow</button></div> : <ol>{entries.map((entry, index) => <li key={`${entry.tool}-${index}`} className={entry.state}><span>{entry.state === "done" ? <Check weight="bold" /> : <CircleNotch className="spin" />}</span><div><strong>{entry.tool}</strong><small>{entry.message}</small></div></li>)}</ol>}
      </div>}
    </aside>
  );
}
